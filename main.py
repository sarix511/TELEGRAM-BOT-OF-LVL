...
async def MaiiiinE():
    ...
    bot_txt_path = os.environ.get("BOT_TXT_PATH", "bot.txt")
    if not os.path.exists(bot_txt_path):
        print(f"❌ {bot_txt_path} not found. Create it with {{\"UID\": \"PASSWORD\"}}")
        return None
    ...
    token_path = os.environ.get("TOKEN_JSON_PATH", "token.json")
